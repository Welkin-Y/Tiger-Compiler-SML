...
val parenthesesDepth : int ref = ref 0
val bracketsDepth : int ref = ref 0
val braceDepth : int ref = ref 0
...
in
    if !commentDepth <> 0 then
        ErrorMsg.error pos ("Comment not terminated.")
    else ();
    if !parenthesesDepth <> 0 then
        ErrorMsg.error pos ("Parentheses are unbalanced.")
    else ();
    if !bracketsDepth <> 0 then
        ErrorMsg.error pos ("Brackets are unbalanced.")
    else ();
    if !braceDepth <> 0 then
        ErrorMsg.error pos ("Braces are unbalanced.")
    else ();
    Tokens.EOF(pos,pos)
end
...
fun yyAction33 (strm, lastMatch : yymatch) = (yystrm := strm;
      (parenthesesDepth := !parenthesesDepth + 1; Tokens.LPAREN(yypos, yypos+1)))
fun yyAction34 (strm, lastMatch : yymatch) = let
    ...
    if !parenthesesDepth=0 then  
        ...
        (parenthesesDepth := !parenthesesDepth - 1; Tokens.RPAREN(yypos, yypos+1))
)
      end
fun yyAction35 (strm, lastMatch : yymatch) = (yystrm := strm;
      (bracketsDepth := !bracketsDepth + 1; Tokens.LBRACK(yypos, yypos+1)))
fun yyAction36 (strm, lastMatch : yymatch) = let
    ...
    if !bracketsDepth = 0 then
        ...
        (bracketsDepth := !bracketsDepth - 1; Tokens.RBRACK(yypos, yypos+1))
)
      end
fun yyAction37 (strm, lastMatch : yymatch) = (yystrm := strm;
      (braceDepth := !braceDepth + 1; Tokens.LBRACE(yypos, yypos+1)))
fun yyAction38 (strm, lastMatch : yymatch) = let
    ...
    if !braceDepth = 0 then
        ...
        (braceDepth := !braceDepth - 1; Tokens.RBRACE(yypos, yypos+1))
)
      end
...
fun yyQ26 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction25(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"="
              then yyQ89(strm', yyMATCH(strm, yyAction25, yyNO_MATCH))
       ...
       of NONE => yyAction27(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #">"
              ...
              then if inp = #"="
                  then yyQ90(strm', yyMATCH(strm, yyAction27, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"#"
              ...
              then if inp = #"\""
                  ...
                  else yyQ96(strm', yyMATCH(strm, yyAction48, yyNO_MATCH))
            else if inp = #"\\"
